...
import { getPaper, listPapers } from '../services/api'
...
export default function PaperProfile() {
  ...
  useEffect(() => {
    ...
    getPaper(decodedId)
      .then(paper => {
        setData(paper)
        // Fetch similar papers using first topic as query
        const firstTopic = paper.topics?.[0] || ''
        if (firstTopic) {
          return listPapers(firstTopic, 5)
            .then(paps => setSimilarPapers(paps.filter(p => p.id !== decodedId)))
        }
      })
  ...
  const authorList = data.authors
    ? data.authors.replace(/^By /, '').split(',').map(a => a.trim()).filter(Boolean)
    : []
    ...
    <div className="pp">
      ...
      <div className="pp-body">
        ...
        {similarPapers.length > 0 && (
          <div className="pp-section">
            <h2 className="pp-section__title">Related Papers</h2>
            <div className="pp-similar">
              {similarPapers.map(p => (
                <div key={p.id} className="pp-similar__card"
...
function ErrorState({ message, onBack }) {
  return (
    <div style={{display:'flex',alignItems:'center',justifyContent:'center',minHeight:'60vh',flexDirection:'column',gap:'1rem',padding:'2rem'}}>
      ...
      <h3 style={{color:'#1e3a5f'}}>Could not load paper</h3>
      <p style={{color:'#64748b'}}>{message}</p>
      <button className="pp-back" onClick={onBack}><ArrowLeftIcon /> Go Back</button>
    </div>
  )
}

/* ── Icons ───────────────────────────────────────────────────── */
function ArrowLeftIcon() { return <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><line x1="19" y1="12" x2="5" y2="12"/><polyline points="12 19 5 12 12 5"/></svg> }
function PaperBigIcon() { return <svg width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="rgba(255,255,255,0.9)" strokeWidth="1.2"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/><line x1="16" y1="17" x2="8" y2="17"/></svg> }
...
function BookIcon() { return <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M4 19.5A2.5 2.5 0 0 1 6.5 17H20"/><path d="M6.5 2H20v20H6.5A2.5 2.5 0 0 1 4 19.5v-15A2.5 2.5 0 0 1 6.5 2z"/></svg> }
function CitationIcon() { return <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><polyline points="23 6 13.5 15.5 8.5 10.5 1 18"/><polyline points="17 6 23 6 23 12"/></svg> }
...
function ExtLinkIcon() { return <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6"/><polyline points="15 3 21 3 21 9"/><line x1="10" y1="14" x2="21" y2="3"/></svg> }